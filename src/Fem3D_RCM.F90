module Reverse_Cuthill_McKee_Node_Reordering
...
!
  ...
  integer ( kind = 8 ), allocatable, dimension ( : ) :: adj
  integer ( kind = 8 ) adj_num
  integer ( kind = 8 ), allocatable, dimension ( : ) :: adj_row
  integer ( kind = 8 ) arg_num
  integer ( kind = 8 ) bandwidth
  logical, parameter :: debug = .false.
  integer ( kind = 8 ) dim_num
  integer ( kind = 8 ) i
  integer ( kind = 8 ) iarg
  integer ( kind = 8 ) iargc
  character ( len = 255 ) :: node_filename = ' '
  character ( len = 255 ) :: element_filename = ' '
  integer ( kind = 8 ) j
  integer ( kind = 8 ) node
  integer ( kind = 8 ) node_num
  ...
  integer ( kind = 8 ), allocatable, dimension ( : ) :: perm_inv
  character ( len = 255 ) prefix
  integer ( kind = 8 ), allocatable, dimension ( :, : ) :: tetra_node
  integer ( kind = 8 ) tetra_num
  integer ( kind = 8 ) tetra_order
...
!
  ...
  call r8mat_transpose_print_some ( dim_num, node_num, node_xyz, int(1,8), int(1,8), &
    dim_num, int(5,8), '  Coordinates of first 5 nodes:' )
!
...
!
  ...
  call i4mat_transpose_print_some ( tetra_order, tetra_num, tetra_node, &
   int(1,8), int(1,8), tetra_order, int(5,8), '  First 5 tetrahedrons:' )
!
...
!  Parameters:
!
!    Input, integer ( kind = 8 ) NODE_NUM, the number of nodes.
!
!    Input, integer ( kind = 8 ) ADJ_NUM, the number of adjacency entries.
!
!    Input, integer ( kind = 8 ) ADJ_ROW(NODE_NUM+1).  Information about row I 
!    is stored in entries ADJ_ROW(I) through ADJ_ROW(I+1)-1 of ADJ.
!
!    Input, integer ( kind = 8 ) ADJ(ADJ_NUM), the adjacency structure.
!    For each row, it contains the column indices of the nonzero entries.
!
!    Output, integer ( kind = 8 ) ADJ_BANDWIDTH, the bandwidth of the adjacency
!    matrix.
!
  implicit none
  ...
  integer ( kind = 8 ) adj(adj_num)
  integer ( kind = 8 ) adj_bandwidth
  integer ( kind = 8 ) adj_row(node_num+1)
  integer ( kind = 8 ) band_hi
  integer ( kind = 8 ) band_lo
  integer ( kind = 8 ) col
  integer ( kind = 8 ) i
  integer ( kind = 8 ) j
  integer ( kind = 8 ) ni
...
!    For each row, it contains the column indices of the nonzero entries.
!
!    Input, integer ( kind = 8 ) PERM(NODE_NUM), integer PERM_INV(NODE_NUM), 
!    the permutation and inverse permutation.
!
!    Output, integer ( kind = 8 ) ADJ_PERM_BANDWIDTH, the bandwidth of the 
...
!
  ...
  integer ( kind = 8 ) adj(adj_num)
  integer ( kind = 8 ) adj_perm_bandwidth
  integer ( kind = 8 ) adj_row(node_num+1)
  integer ( kind = 8 ) band_hi
  integer ( kind = 8 ) band_lo
  integer ( kind = 8 ) col
  ...
  integer ( kind = 8 ) ni
  integer ( kind = 8 ) nj
  integer ( kind = 8 ) perm(node_num)
  integer ( kind = 8 ) perm_inv(node_num)
  integer ( kind = 8 ) pi
  integer ( kind = 8 ) pj
...
!    Input, integer ( kind = 8 ) ADJ_NUM, the number of adjacency entries.
!
!    Input, integer ( kind = 8 ) ADJ_ROW(NODE_NUM+1), organizes the adjacency
!    entries into rows.  The entries for row I are in entries ADJ_ROW(I)
!    through ADJ_ROW(I+1)-1.
!
!    Input, integer ( kind = 8 ) ADJ(ADJ_NUM), the adjacency structure, which
...
!
  ...
  call adj_print_some ( node_num, int(1,8), node_num, adj_num, adj_row, adj )
...
!    Input, integer ( kind = 8 ) NODE_NUM, the number of nodes.
!
!    Input, integer ( kind = 8 ) NODE_LO, NODE_HI, the first and last nodes for
...
!
  ...
  integer ( kind = 8 ) jmax
  integer ( kind = 8 ) jmin
  integer ( kind = 8 ) node_hi
  integer ( kind = 8 ) node_lo
...
!    are legal.
!
!    Output, integer ( kind = 8 ) DIGIT, the corresponding value.  If C was
...
!  Parameters:
!
!    Input, integer ( kind = 8 ) ROOT, the node that defines the connected
!    component.
!
!    Input, integer ( kind = 8 ) ADJ_NUM, the number of adjacency entries.
!
...
!    For each row, it contains the column indices of the nonzero entries.
!
!    Input, integer ( kind = 8 ) MASK(NODE_NUM), is nonzero for those nodes 
!    which are to be considered.
!
!    Output, integer ( kind = 8 ) DEG(NODE_NUM), contains, for each node in 
!    the connected component, its degree.
!
!    Output, integer ( kind = 8 ) ICCSIZE, the number of nodes in the 
!    connected component.
!
!    Output, integer ( kind = 8 ) LS(NODE_NUM), stores in entries 1 through
...
!
  ...
  integer ( kind = 8 ) adj(adj_num)
  integer ( kind = 8 ) adj_row(node_num+1)
  integer ( kind = 8 ) deg(node_num)
  integer ( kind = 8 ) i
  integer ( kind = 8 ) iccsze
  integer ( kind = 8 ) ideg
  integer ( kind = 8 ) j
  integer ( kind = 8 ) jstop
  integer ( kind = 8 ) jstrt
  integer ( kind = 8 ) lbegin
  integer ( kind = 8 ) ls(node_num)
  integer ( kind = 8 ) lvlend
  integer ( kind = 8 ) lvsize
  integer ( kind = 8 ) mask(node_num)
  integer ( kind = 8 ) nbr
  integer ( kind = 8 ) node
  integer ( kind = 8 ) root
!
...
!
!    Output, integer ( kind = 8 ) COLUMN_NUM, the number of columns in the file.
!
  implicit none

  integer ( kind = 8 ) column_num
  logical got_one
  character ( len = * ) input_filename
  integer ( kind = 8 ) input_unit
  integer ( kind = 8 ) ios
...
!
!    Output, integer ( kind = 8 ) ROW_NUM, the number of rows found.
!
  implicit none

  integer ( kind = 8 ) bad_num
  integer ( kind = 8 ) comment_num
  integer ( kind = 8 ) ierror
  character ( len = * ) input_filename
  integer ( kind = 8 ) input_unit
  integer ( kind = 8 ) ios
  character ( len = 255 ) line
  integer ( kind = 8 ) record_num
  integer ( kind = 8 ) row_num
...
!    Input, integer ( kind = 8 ) ADJ_NUM, the number of adjacency entries.
!
!    Input, integer ( kind = 8 ) ADJ_ROW(NODE_NUM+1).  Information about 
!    row I is stored in entries ADJ_ROW(I) through ADJ_ROW(I+1)-1 of ADJ.
!
!    Input, integer ( kind = 8 ) ADJ(ADJ_NUM), the adjacency structure.
!    For each row, it contains the column indices of the nonzero entries.
!
!    Output, integer ( kind = 8 ) PERM(NODE_NUM), the RCM ordering.
!
!  Local Parameters:
!
...
!
  ...
  integer ( kind = 8 ) iccsze
  integer ( kind = 8 ) mask(node_num)
  integer ( kind = 8 ) level_num
  integer ( kind = 8 ) level_row(node_num+1)
  integer ( kind = 8 ) num
  integer ( kind = 8 ) perm(node_num)
  integer ( kind = 8 ) root
...
!  Parameters:
!
!    Output, integer ( kind = 8 ) IUNIT, the free unit number.
!
  implicit none
...
!  Parameters:
!
!    Input/output, integer ( kind = 8 ) I, J.  On output, the values of I and
...
!  Parameters:
!
!    Input, integer ( kind = 8 ) M, N, the number of rows and columns.
!
!    Input, integer ( kind = 8 ) A(M,N), an array of N columns of vectors 
!    of length M.
!
!    Input, integer ( kind = 8 ) I, J, the columns to be compared.
!    I and J must be between 1 and N.
!
!    Output, integer ( kind = 8 ) ISGN, the results of the comparison:
!    -1, column I < column J,
!     0, column I = column J,
!    +1, column J < column I.
!
...
!  Parameters:
!
!    Input, integer ( kind = 8 ) M, the number of rows of A, and the length of
!    a vector of data.
!
!    Input, integer ( kind = 8 ) N, the number of columns of A.
!
!    Input/output, integer ( kind = 8 ) A(M,N).
...
!  Parameters:
!
!    Input, integer ( kind = 8 ) M, the number of rows of A.
!
!    Input, integer ( kind = 8 ) N, the number of columns of A, and the length
!    of a vector of data.
!
!    Input/output, integer ( kind = 8 ) A(M,N).
!    On input, the array of N columns of M vectors.
...
!    Input, integer ( kind = 8 ) M, N, the number of rows and columns.
!
!    Input, integer ( kind = 8 ) A(M,N), a sorted array, containing
!    N columns of data.
!
!    Output, integer ( kind = 8 ) UNIQUE_NUM, the number of unique columns.
!
  implicit none
...
!  Parameters:
!
!    Input, integer ( kind = 8 ) M, N, the number of rows and columns in 
!    the array.
!
!    Input/output, integer ( kind = 8 ) A(M,N), an array of N columns 
!    of length M.
!
!    Input, integer ( kind = 8 ) J1, J2, the columns to be swapped.
!
  implicit none
...
!
!    Input, integer ( kind = 8 ) N, the number of points.
!
!    Output, integer ( kind = 8 ) TABLE(M,N), the table data.
!
  implicit none
  ...
  integer ( kind = 8 ) ierror
  character ( len = * ) input_filename
  integer ( kind = 8 ) input_status
  integer ( kind = 8 ) input_unit
  integer ( kind = 8 ) j
  character ( len = 255 ) line
  integer ( kind = 8 ) table(m,n)
  integer ( kind = 8 ) x(m)
...
!    Input, character ( len = * ) INPUT_FILENAME, the name of the input file.
!
!    Output, integer ( kind = 8 ) M, spatial dimension.
!
!    Output, integer ( kind = 8 ) N, the number of points.
!
  implicit none
...
!    Input, integer ( kind = 8 ) M, N, the number of rows and columns.
!
!    Input, integer ( kind = 8 ) A(M,N), an M by N matrix to be printed.
!
!    Input, integer ( kind = 8 ) ILO, JLO, the first row and column to print.
!
!    Input, integer ( kind = 8 ) IHI, JHI, the last row and column to print.
!
!    Input, character ( len = * ) TITLE, an optional title.
!
  implicit none

  integer ( kind = 8 ), parameter :: incx = 10
  integer ( kind = 8 ) m
  integer ( kind = 8 ) n
...
!    Input, integer ( kind = 8 ) N, the number of points.
!
!    Input, integer ( kind = 8 ) TABLE(M,N), the table data.
!
  implicit none
  ...
  integer ( kind = 8 ) output_status
  integer ( kind = 8 ) output_unit
...
!
  do j = 1, n
    write ( output_unit, '(*(I8))' ) table(1:m,j)
  end do
!
!  Close the file.
!
...
!    Input, integer ( kind = 8 ) M, N, the number of rows and columns.
!
!    Input, integer ( kind = 8 ) A(M,N), an array of M rows of vectors 
!    of length N.
!
!    Input, integer ( kind = 8 ) I, J, the rows to be compared.
...
!    Input, integer ( kind = 8 ) M, N, the number of rows and columns.
!
!    Input/output, integer ( kind = 8 ) A(M,N), an array of data.
!
!    Input, integer ( kind = 8 ) IROW1, IROW2, the two rows to swap.
!
  implicit none
  ...
  integer ( kind = 8 ) a(m,n)
  integer ( kind = 8 ) irow1
  integer ( kind = 8 ) irow2
  integer ( kind = 8 ) row(n)
!
!  Check.
!
...
!  Parameters:
!
!    Input, integer ( kind = 8 ) N, the number of components of the vector.
!
...
!  Parameters:
!
!    Input, integer ( kind = 8 ) N, the number of entries in the array.
!
!    Input/output, integer ( kind = 8 ) A(N), the array to be reversed.
!
  implicit none
...
!  Parameters:
!
!    Input, integer ( kind = 8 ) ROOT, the node at which the level structure
!    is to be rooted.
!
!    Input, integer ( kind = 8 ) ADJ_NUM, the number of adjacency entries.
!
...
!
  ...
  integer ( kind = 8 ) element_num
  integer ( kind = 8 ) element_order

  integer ( kind = 8 ) element
  integer ( kind = 8 ) element_node(element_order,element_num)
  integer ( kind = 8 ) node
  integer ( kind = 8 ) node_max
  integer ( kind = 8 ) node_min
  integer ( kind = 8 ) node_num
...
!  Parameters:
!
!    Input, integer ( kind = 8 ) N, the number of entries.
!
!    Input, integer ( kind = 8 ) P(N), the array to check.
!
...
!  Parameters:
!
!    Input, integer ( kind = 8 ) N, the number of items permuted.
!
!    Input, integer ( kind = 8 ) PERM(N), a permutation.
!
!    Output, integer ( kind = 8 ) PERM_INV(N), the inverse permutation.
!
  implicit none
  ...
  integer ( kind = 8 ) i
  integer ( kind = 8 ) perm(n)
  integer ( kind = 8 ) perm_inv(n)
...
!  Parameters:
!
!    Input, integer ( kind = 8 ) M, the dimension of objects.
!
!    Input, integer ( kind = 8 ) N, the number of objects.
!
!    Input/output, real ( kind = 8 ) A(M,N), the array to be permuted.
!
!    Input, integer ( kind = 8 ) P(N), the permutation.  P(I) = J means
...
!
  ...
  integer ( kind = 8 ), parameter :: incx = 5
  integer ( kind = 8 ) m
  integer ( kind = 8 ) n
...
!    For each row, it contains the column indices of the nonzero entries.
!
!    Input/output, integer ( kind = 8 ) MASK(NODE_NUM), a mask for the nodes.
...
!    Output, integer ( kind = 8 ) PERM(NODE_NUM), the RCM ordering.
!
!    Output, integer ( kind = 8 ) ICCSZE, the size of the connected component
...
!  Parameters:
!
!    Input/output, integer ( kind = 8 ) ROOT.  On input, ROOT is a node in the
...
!    For each row, it contains the column indices of the nonzero entries.
!
!    Input, integer ( kind = 8 ) MASK(NODE_NUM), specifies a section subgraph.
!    Nodes for which MASK is zero are ignored by FNROOT.
!
!    Output, integer ( kind = 8 ) LEVEL_NUM, is the number of levels in the 
!    level structure rooted at the node ROOT.
!
!    Output, integer ( kind = 8 ) LEVEL_ROW(NODE_NUM+1), 
...
!
  ...
  integer ( kind = 8 ) jstrt
  integer ( kind = 8 ) k
  integer ( kind = 8 ) kstop
  integer ( kind = 8 ) kstrt
  integer ( kind = 8 ) level(node_num)
  integer ( kind = 8 ) level_num
  integer ( kind = 8 ) level_num2
  integer ( kind = 8 ) level_row(node_num+1)
  integer ( kind = 8 ) mask(node_num)
  integer ( kind = 8 ) mindeg
  integer ( kind = 8 ) nabor
  integer ( kind = 8 ) ndeg
  integer ( kind = 8 ) node
  integer ( kind = 8 ) root
!
...
!
!    Output, integer ( kind = 8 ) IVAL, the integer value read from the string.
!    If the string is blank, then IVAL will be returned 0.
!
!    Output, integer ( kind = 8 ) IERROR, an error flag.
!    0, no error.
!    1, an error occurred.
!
!    Output, integer ( kind = 8 ) LENGTH, the number of characters of S 
!    used to make IVAL.
!
  implicit none
...
!
!    Input, integer ( kind = 8 ) N, the number of values expected.
!
!    Output, integer ( kind = 8 ) IVEC(N), the values read from the string.
!
!    Output, integer ( kind = 8 ) IERROR, error flag.
...
!
!    Output, integer ( kind = 8 ) LENGTH, the number of characters read
...
!
  ...
  real ( kind = 8 ) dval
  integer ( kind = 8 ) ierror
  integer ( kind = 8 ) ihave
  integer ( kind = 8 ) isgn
  integer ( kind = 8 ) iterm
  integer ( kind = 8 ) jbot
  integer ( kind = 8 ) jsgn
  integer ( kind = 8 ) jtop
  integer ( kind = 8 ) length
...
!
!    Output, integer ( kind = 8 ) NWORD, the number of "words" in the string.
...
!  Parameters:
!
!    Input, integer ( kind = 8 ) N, the number of items to be sorted.
!
!    Input/output, integer ( kind = 8 ) INDX, the main communication signal.
!
...
!
!    Output, integer ( kind = 8 ) I, J, the indices of two items.
!    On return with INDX positive, elements I and J should be interchanged.
!    On return with INDX negative, elements I and J should be compared, and
!    the result reported in ISGN on the next call.
!
!    Input, integer ( kind = 8 ) ISGN, results of comparison of elements
...
!
  ...
  integer ( kind = 8 ) i
  integer ( kind = 8 ), save :: i_save = 0
  integer ( kind = 8 ) indx
  integer ( kind = 8 ) isgn
  integer ( kind = 8 ) j
  integer ( kind = 8 ), save :: j_save = 0
  integer ( kind = 8 ), save :: k = 0
  integer ( kind = 8 ), save :: k1 = 0
  integer ( kind = 8 ) n
  integer ( kind = 8 ), save :: n1 = 0
!
...
!    Input, integer ( kind = 8 ) NODE_NUM, the number of nodes.
!
!    Input, integer ( kind = 8 ) TETRA_NUM, the number of tetrahedrons.
!
!    Input, integer ( kind = 8 ) TETRA_NODE(4,TETRA_NUM), the indices of 
!    the nodes.
!
!    Output, integer ( kind = 8 ) ADJ_NUM, the total number of adjacency
!    relationships,
!
!    Output, integer ( kind = 8 ) ADJ_ROW(NODE_NUM+1), the ADJ pointer array.
!
  implicit none
  ...
  integer ( kind = 8 ) j
  integer ( kind = 8 ) k
  integer ( kind = 8 ), allocatable ::  pair(:,:)
  integer ( kind = 8 ) pair_num
  integer ( kind = 8 ) pair_unique_num
  integer ( kind = 8 ) tetra_node(4,tetra_num)
!
!  Each order 4 tetrahedron defines 6 adjacency pairs.
!
  allocate(pair(2,6*tetra_num))
...
!
  call i4col_sort2_a ( int(2,8), pair_num, pair )
!
!  Rearrange the columns in ascending order.
!
  call i4col_sort_a ( int(2,8), pair_num, pair )
!
!  Get the number of unique columns.
!
  call i4col_sorted_unique_count ( int(2,8), pair_num, pair, pair_unique_num )
!
...
!    the nodes.
!
!    Input, integer ( kind = 8 ) ADJ_NUM, the total number of adjacency
!    relationships,
!
!    Input, integer ( kind = 8 ) ADJ_ROW(NODE_NUM+1), the ADJ pointer array.
!
!    Output, integer ( kind = 8 ) ADJ(ADJ_NUM), the adjacency information.
!
  implicit none
  ...
  integer ( kind = 8 ) adj(adj_num)
  integer ( kind = 8 ) adj_row(node_num+1)
  integer ( kind = 8 ) adj_row_copy(node_num+1)
  integer ( kind = 8 ) i
  integer ( kind = 8 ) j
  integer ( kind = 8 ) k
  integer ( kind = 8 ), allocatable ::  pair(:,:)
...
!    Input, integer ( kind = 8 ) NODE_NUM, the number of nodes.
!
!    Input, integer ( kind = 8 ) TET_NUM, the number of tetrahedrons.
!
!    Input, integer ( kind = 8 ) TET_NODE(10,TET_NUM), the indices of 
!    the nodes.
!
!    Output, integer ( kind = 8 ) ADJ_NUM, the total number of adjacency
!    relationships,
...
!
  ...
  integer ( kind = 8 ) j
  integer ( kind = 8 ) k
  integer ( kind = 8 ) pair(2,45*tet_num)
  integer ( kind = 8 ) pair_num
  integer ( kind = 8 ) pair_unique_num
  integer ( kind = 8 ) tet_node(10,tet_num)
!
...
end module Reverse_Cuthill_McKee_Node_Reordering