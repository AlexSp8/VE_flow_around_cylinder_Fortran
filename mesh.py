...
Partition_1 = geompy.MakePartition([Face_1, Face_2, Disk], [], [], [], geompy.ShapeType["FACE"], 0, [], 0)

for i in range(0,len(Lines)):
  Partition_1 = geompy.MakePartition([Partition_1, Lines[i]], [], [], [], geompy.ShapeType["FACE"], 0, [], 0)

# geompy.addToStudy(Partition_1, 'Partition_1')
# Face_4 = geompy.MakeTranslation(Face_2, 0, -H/2, z)
# geompy.addToStudy(Face_4, 'Face_4')
# Partition_1 = geompy.MakeCutList(Partition_1, [Face_4, Disk], True)
Partition_1 = geompy.MakeCutList(Partition_1, [Disk], True)
Partition_1 = geompy.MakeTranslation(Partition_1, 0, 0, +W)
Partition_1 = geompy.MakePrismVecH(Partition_1, OZ, -2*W)
geompy.addToStudy(Partition_1, 'Partition_1')
########################################################################
...
Group = geompy.CreateGroup(Partition_1, geompy.ShapeType["EDGE"])
vertex = geompy.MakeVertex(L1, 0.0, 0.0)
#get sub-shape (edge)
edge = geompy.GetShapesNearPoint(Partition_1, vertex, geompy.ShapeType["EDGE"])
#edge ID
ID = geompy.GetSubShapeID(Partition_1, edge)
...
vertex = geompy.MakeVertex(-L1, e, 0.0)
#get sub-shape (edge)
...
geompy.addToStudyInFather(Partition_1, Group, 'Inflow_2D')
Groups.append(Group)
j = j+1
...
geompy.addToStudyInFather(Partition_1, Group, 'Outflow_2D')
Groups.append(Group)
j = j+1
...
for i in range(0,len(x)):
  #up
  vertex = geompy.MakeVertex(x[i], y[i], 0.0)
  face = geompy.GetShapesNearPoint(Partition_1, vertex, geompy.ShapeType["FACE"])
...
for k in range(len(y)):
  ...
  for i in range(len(x)):
    vertex = geompy.MakeVertex(x[i], y[k], 0.0)
    #get sub-shape (edge)
    edge = geompy.GetShapesNearPoint(Partition_1, vertex, geompy.ShapeType["FACE"])
    #edge ID
    ID = geompy.GetSubShapeID(Partition_1, edge)
...
for i2 in range(0,2):
  ...
  for k in range(1,3):
    for i in range(len(x)):
      yi = y[i]*((-1)**k)
      vertex = geompy.MakeVertex(x[i], yi, z[i2])
  ...
  geompy.addToStudyInFather(Partition_1, Group, name[i2])
  Groups.append(Group)
  j = j+1
...
size = len(Groups)
# print(size)
Inflow_1D = Mesh_1.GroupOnGeom(Groups[size-8],'Inflow_1D')
Inflow_2D = Mesh_1.GroupOnGeom(Groups[size-7],'Inflow_2D')
Outflow_2D = Mesh_1.GroupOnGeom(Groups[size-6],'Outflow_2D')
Cylinder = Mesh_1.GroupOnGeom(Groups[size-5],'Cylinder')
Top_wall = Mesh_1.GroupOnGeom(Groups[size-4],'Top_wall')
Bottom_wall = Mesh_1.GroupOnGeom(Groups[size-3],'Bottom_wall')
Front = Mesh_1.GroupOnGeom(Groups[size-2],'Front')
Back = Mesh_1.GroupOnGeom(Groups[size-1],'Back')
...
Number_of_Segments.SetScaleFactor(Factors[j])
smesh.SetName(Number_of_Segments, 'Number of Scaled Segments_'+str(k))
Number_of_Segments.SetConversionMode( 0 )
# Number_of_Segments.SetReversedEdges( [] )
Number_of_Segments.SetExpressionFunction( '0*exp(-(t-0.5)^2)' )
Propagation_of_Node = Regular_1D.PropagationOfDistribution()
smesh.SetName(Propagation_of_Node, 'Propagation of Node Distribution on Opposite Edges')
# Number_of_Segments.SetObjectEntry('Partition_1')
Sub_mesh = Regular_1D.GetSubMesh()
...
# if (Quadratic):
#   Mesh_1.ConvertToQuadratic(0) #medium node on the edge (geometry)
...
try:
  ...
  # Mesh_1.ExportDAT(r'/home/aspyridakis/Downloads/Cylinder.DTA', Cylinder)
  # Mesh_1.ExportDAT(r'/home/aspyridakis/Downloads/Top_wall.DTA', Top_wall)
  # Mesh_1.ExportDAT(r'/home/aspyridakis/Downloads/Bottom_wall.DTA', Bottom_wall)
